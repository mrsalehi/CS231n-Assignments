...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        dout, dW_vocab, db_vocab = temporal_affine_backward(dout, cache_temporal_affine)
    ...
    def sample(self, features, max_length=30):
        ...
        for i in range(max_length):
            ...
            if self.cell_type == 'rnn':
                h, _ = rnn_step_forward(prev_word_embed.reshape(N, -1), prev_h, Wx, Wh, b)    
            else:
                h, c, _ = lstm_step_forward(prev_word_embed.reshape(N, -1), prev_h, prev_c, Wx, Wh, b)
                prev_c = c
            next_word, _ = affine_forward(h, W_vocab, b_vocab)
            next_word = np.argmax(next_word, axis=1)
            prev_h = h
            captions[:, i] = next_word.reshape(N)
            prev_word = next_word[:, np.newaxis]
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        return captions
