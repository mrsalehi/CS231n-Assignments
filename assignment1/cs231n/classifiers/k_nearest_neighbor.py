...
class KNearestNeighbor(object):
    ...
    def compute_distances_no_loops(self, X):
        ...
        #########################################################################
        X1 = np.transpose([np.sum(np.power(X, 2), axis=1)] * self.X_train.shape[0])
        X2 = np.array([np.sum(np.power(self.X_train, 2), axis=1)] * X.shape[0])
        dists = np.sqrt(-2 * np.matmul(X, np.transpose(self.X_train)) + X1 + X2)
        #########################################################################
        #                         END OF YOUR CODE                              #
        #########################################################################
        return dists
